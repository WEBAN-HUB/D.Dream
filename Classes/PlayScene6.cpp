...
bool PlayScene6::init()
{
	...
	auto closeItem = MenuItemImage::create(
		"CloseNormal.png",
		"CloseSelected.png",
		CC_CALLBACK_1(PlayScene6::menuCloseCallback, this));
	...
	auto label = Label::createWithTTF("Vacation", "fonts/bemin.ttf", 25);
	...
	Singleton_data::getInstance()->Stage = 6;
	...
	tKeyListener->onKeyPressed = CC_CALLBACK_2(PlayScene6::onKeyPressed, this);
	tKeyListener->onKeyReleased = CC_CALLBACK_2(PlayScene6::onKeyReleased, this);
	...
	mpListener->onTouchBegan = CC_CALLBACK_2(PlayScene6::onTouchBegan, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(mpListener, this);


	multiListener = EventListenerTouchAllAtOnce::create();
	multiListener->onTouchesBegan = CC_CALLBACK_2(PlayScene6::onTouchesBegan, this);
	...
	this->schedule(schedule_selector(PlayScene6::OnTimer), 0.11111111);
	this->schedule(schedule_selector(PlayScene6::kicktimer), 0.11111111);
	this->scheduleOnce(schedule_selector(PlayScene6::snaretimer), 1.77777776);
	this->schedule(schedule_selector(PlayScene6::bartimer), 0.5);
	this->scheduleUpdate();

	return true;
}

void PlayScene6::bartimer(float t)
{
...
void PlayScene6::onKeyPressed(cocos2d::EventKeyboard::KeyCode keycode, cocos2d::Event* event)
{
...
void PlayScene6::onKeyReleased(cocos2d::EventKeyboard::KeyCode keycode, cocos2d::Event* event)
{
	if (keycode == EventKeyboard::KeyCode::KEY_LEFT_ARROW)
	{
...
bool PlayScene6::onTouchBegan(cocos2d::Touch* touch, cocos2d::Event* unused_event)
{
...
void PlayScene6::onTouchesBegan(const std::vector<Touch*>& touches, Event* event)
{
	for (int ti = 0; ti < 3; ti++)
	{
...
void PlayScene6::kicktimer(float t)
{
	kick_count++;
...
void PlayScene6::snaretimer(float t)
{
	snarenote->DoAlive();
...
void PlayScene6::OnTimer(float  f)
{
...
void PlayScene6::update_stat(float dt)
{
	char tempz[512];
	memset(tempz, 0, 512);
...
{
	/*this->schedule(schedule_selector(PlayScene6::OnTimer), 1);
	normal_count = 0;*/



	this->unscheduleUpdate();
	this->unschedule(schedule_selector(PlayScene6::OnTimer));
