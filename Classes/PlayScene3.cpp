...
bool PlayScene3::init()
{
	...
	auto label = Label::createWithTTF("4Beats Hard\nPractice", "fonts/bemin.ttf", 25);
	...
	Singleton_data::getInstance()->Life = 8;
	Singleton_data::getInstance()->Score = 0;
	Singleton_data::getInstance()->Time = 0;
	Singleton_data::getInstance()->Stage = 3;
	...
	tKeyListener->onKeyPressed = CC_CALLBACK_2(PlayScene3::onKeyPressed, this);
	tKeyListener->onKeyReleased = CC_CALLBACK_2(PlayScene3::onKeyReleased, this);
	...
	mpListener->onTouchBegan = CC_CALLBACK_2(PlayScene3::onTouchBegan, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(mpListener, this);


	multiListener = EventListenerTouchAllAtOnce::create();
	multiListener->onTouchesBegan = CC_CALLBACK_2(PlayScene3::onTouchesBegan, this);
	...
	this->schedule(schedule_selector(PlayScene3::OnTimer), 0.22222222);
	this->schedule(schedule_selector(PlayScene3::kicktimer),0.22222222);
	this->scheduleOnce(schedule_selector(PlayScene3::snaretimer), 1.77777776);
	this->scheduleUpdate();
...
void PlayScene3::onKeyPressed(cocos2d::EventKeyboard::KeyCode keycode, cocos2d::Event* event)
{
...
bool PlayScene3::onTouchBegan(cocos2d::Touch* touch, cocos2d::Event* unused_event)
{
...
void PlayScene3::onTouchesBegan(const std::vector<Touch*>& touches, Event* event)
{
	for (int ti = 0; ti < 3; ti++)
	{
...
void PlayScene3::onKeyReleased(cocos2d::EventKeyboard::KeyCode keycode, cocos2d::Event* event)
{
	if (keycode == EventKeyboard::KeyCode::KEY_LEFT_ARROW)
	{
...
void PlayScene3::kicktimer(float t)
{
	kick_count++;
	if (kick_count == 4)
	{
...
void PlayScene3::snaretimer(float t)
{
	snarenote->DoAlive();
...
void PlayScene3::OnTimer(float  f)
{
...
void PlayScene3::update_stat(float dt)
{
	char tempz[512];
	memset(tempz, 0, 512);
...
{
	/*this->schedule(schedule_selector(PlayScene3::OnTimer), 1);
	normal_count = 0;*/



	this->unscheduleUpdate();
	this->unschedule(schedule_selector(PlayScene3::OnTimer));
